...
class TrackGrid extends Component {
    ...
    componentDidUpdate(prevProps) {
        ...
        if(this.props.correctTrack !== prevProps.correctTrack && this.props.correctTrack !== null) {
            setTimeout(() => {
    ...
    randomlyShowTracks = () => {
        ...
        if (remainingKeys.length > 0) {
            const keyToShow = remainingKeys[Math.floor(Math.random() * remainingKeys.length)];
            const newHiddenState = update(this.state.hide, { [keyToShow]: { $set: false } });
    ...
    renderTracks = () => {
        if(this.props.tracks) {
            ...
            this.props.tracks.map((el, i) => {
                tracks.push(<Track gridindex={i} bigTrack={this.state.correctTrack === el.id ? true : false} info={el} key={el.id} showingNames={this.state.showingNames} hide={this.state.hide[el.id]} handleClick={this.handleClick} />)
            });
            return tracks;
        }
    }
    ...
    getCorrectTrackDetails = () => {
        const correctTrackDetails = this.props.tracks.find((el) => {return el.id === this.props.correctTrack});
    ...
    renderBottomInfo = () => {
        if(this.state.displayAnswerText) {
            ...
            if(this.state.selectedTrack === this.state.correctTrack) {
                return (
                    <div className="NextSongButton" onClick={this.nextButtonPressed}><NextButton>Next Song</NextButton></div>
                );
            } else if(this.props.lives === 0) {
                return (
                    <div>
                        <div className="AttemptsRemaining">No attempts remaining</div>
                        <div className="NextSongButton" onClick={this.nextButtonPressed}><NextButton>Get Your Results</NextButton></div>
                    </div>
                );
            } else {
                return (
                    <div>
                        <div className="AttemptsRemaining">{this.props.lives} attempt{this.props.lives === 1 ? null : "s"} remaining</div>
                        <div className="NextSongButton" onClick={this.nextButtonPressed}><NextButton>Next Song</NextButton></div>
                    </div>
                );
            }
    ...
    render() {
        return (
            <div>
            <div className={`TracksWrapper ${this.state.correctTrack !== null ? "ShowingAnswer" : ""}`}>
                {this.renderTracks()}
                {this.state.displayAnswerText ? <Answer correctTrackDetails={this.getCorrectTrackDetails()} correctTrack={this.state.correctTrack} selectedTrack={this.state.selectedTrack} /> : null}
            </div>
                {this.state.displayAnswerText ? <NextButton onClick={this.nextButtonPressed} correctTrack={this.state.correctTrack} selectedTrack={this.state.selectedTrack} lives={this.props.lives}/> : null}
